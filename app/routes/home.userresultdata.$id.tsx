      ...
      <div className="grow  p-4 w-full">
        <div className="flex">
          ...
          <div className="flex gap-2">
            <Link
              to={`/home/userresult/${data.data.searchResult[0].projectId}`}
          ...
          <>
            {data.data.searchResult[0].assesement.result.map(
              (val: any, index: number) => (
                <div
                  key={index}
                  className="bg-secondary px-6 py-3 rounded-lg my-6 backdrop-filter backdrop-blur-lg bg-opacity-20 border-2 border-secondary"
                >
