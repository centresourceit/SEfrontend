...
const AddQuestion: React.FC = (): JSX.Element => {
  ...
  const [answers, setAnswers] = useState<
    { answer: string; mark: number; rec: string }[]
  >([
    {
      answer: "",
      mark: 0,
  ...
  const addAnserField = () => {
    if (qType.current?.value == "0")
      return toast.error("First select the question type.", { theme: "light" });
    if (answers.length > 10)
      toast.error("You already created max now on answer.", { theme: "light" });
    if (qType.current?.value == "TANDF") {
      if (answers.length < 2) {
        ...
        toast.error("True and false only create two answer.", {
          theme: "light",
        });
      }
    } else {
  ...
  const addQuestion = async () => {
    const AnswerScheme = z
      .object({
        answer: z.string().nonempty("Enter Answer in all fields"),
        mark: z.number({
          required_error: "Enter Mark in all fields",
          invalid_type_error: "Select valid the mark",
        }),
        rec: z.string().nonempty("Enter recommendation in all fields"),
      })
      .strict();
    ...
    const QuestionScheme = z
      .object({
        ...
        licensesId: z
          ...
          .refine((val) => val != 0, {
            message: "Select the Question License Plan",
          }),
        ...
        questioncode: z.string().nonempty("Question code is required"),
    ...
    <>
      <div className="grow w-full  p-4  ">
        ...
        {licenses == null || licenses == undefined ? (
          ...
          <p className="text-rose-500 font-semibold text-2xl my-4 rounded-md border-l-4 px-2 py-2 bg-rose-500 bg-opacity-20 border-rose-500 w-full">
            There is no principles. Create question in order to create
            principle.
          </p>
        ) : (
          <>
            ...
            >
              ...
              <option className=" text-white text-lg" value="MCQ">
                MCQ
              </option>
              <option className=" text-white text-lg" value="SLIDER">
                SLIDER
              </option>
              <option className=" text-white text-lg" value="TANDF">
                TANDF
              </option>
              <option className=" text-white text-lg" value="PERCENTAGE">
                PERCENTAGE
              </option>
            </select>
            <h2 className="text-white font-semibold text-md">
            ...
            {answers.map(
              (
                val: { answer: string; mark: number | string; rec: string },
                index: number
              ) => (
                <div
                  key={index}
                  className="p-4 bg-primary-800 my-4 bg-opacity-80 rounded-mds"
                >
                  <h2 className="text-white font-semibold text-md">
                    <span className="text-green-500 pr-2">&#x2666;</span>
                    Answer {index + 1}
