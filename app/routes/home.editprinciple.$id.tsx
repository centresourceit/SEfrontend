...
export async function loader({ params, request }: LoaderArgs) {
    ...
    const data = await ApiCall({
        query: `
        query getPrincipleById($id: Int!){
            getPrincipleById(id: $id){
                id,
                name,
                description
            },
    ...
    return json({ token: cookie.token, userId: cookie.id, principle: data.data.getPrincipleById });
}


const EditPrinciple: React.FC = (): JSX.Element => {
    ...
    useEffect(() => {
        pName!.current!.value = principle.name;
        pDesciption!.current!.value = principle.description;
    }, []);
    ...
    const updatePrinciple = async () => {
        const PrincipleScheme = z
            .object({
                name: z
                    .string()
                    .nonempty("Principle Name is required."),
                description: z
                    .string()
                    .nonempty("Principle Description is required")
            })
            .strict();
