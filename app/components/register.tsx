    ...
    const submit = () => {
        const RegisterScheme = z.object({
            ...
            password: z
                ...
                .regex(/[A-Z]/, "Password should be contain atleast one Capital letter.")
                .regex(/[a-z]/, "Password should be contain atleast one lower letter.")
                .regex(/\d/, "Password should be contain atleast one degit letter.")
                .regex(/[@$!%*?&]/, "Password should be contain atleast one specil character [@$!%*?&]."),
            repassword: z
                .string()
                .nonempty("Re-Password is required")
                .min(8, "Re-Password should be have atlest 8 character.")
                .regex(/[A-Z]/, "Re-Password should be contain atleast one Capital letter.")
                .regex(/[a-z]/, "Re-Password should be contain atleast one lower letter.")
                .regex(/\d/, "Re-Password should be contain atleast one degit letter.")
                .regex(/[@$!%*?&]/, "Re-Password should be contain atleast one specil character [@$!%*?&]."),
        }).strict().refine((val) => {
    ...
    return (
        <>
            <section className="h-screen w-full relative">
                <img src="/images/bg/bg4.png" alt="error" className="scale-x-[-1] scale-y-[-1] absolute top-0 left-0 w-full h-screen object-cover object-center" />
                <div className="grid place-items-center bg-[#181136] h-screen w-full">
                    <div className="z-50 w-80">
                        ...
                        <div className="border-b-2 border-gray-200 py-1">
                            ...
                            <input ref={emaliRef} type="text" placeholder="Email" className="bg-transparent outline-none border-none fill-none text-white" />
                        </div>
                        ...
                        <div className="border-b-2 border-gray-200 py-1 mt-4">
                            <FontAwesomeIcon className="text-white fotn-bold text-xl mr-4" icon={showRePassword ? faEye : faEyeSlash} onClick={handelRePassword}></FontAwesomeIcon>
                            <input ref={rePassRef} type={showRePassword ? "text" : "password"} placeholder="Re-Password" className="bg-transparent outline-none border-none fill-none text-white" />
                        </div>
                        <button onClick={submit} className="text-[#181136] bg-[#54edec] py-2 px-6 text-md font-medium rounded-full w-full mt-6">Register</button>
                        <h5 className="text-white text-center mt-8">Already have an account? <Link to={"/login"} className="text-[#54edec]">Sign In</Link></h5>
